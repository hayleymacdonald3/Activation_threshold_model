...
To run Go and GS trials with a single facilitation curve: python ARI_task.py first
To run GS trials with an additional facilitation curve using manual input
for optimized parameters from single facilitation curve: python ARI_task.py second
...
def get_facNew(t, params_new, facBimanual):
    ...
    fac2 = np.add(
        fac1, (a_facGo * np.exp(-(t - b_facGo)**2 / (2 * c_facGo**2))))
    return fac2
...
def get_emg_onsets_offsets(t, fac_i, inhib):
    ...
    for i in range(ntrials):
        if np.all(switches[i] == 1):
            emg_onsets = np.append(
                emg_onsets, (1000 * (inhib[i, 1] - fac_i[i].max()) + t[np.argmax(fac_i[i])]))
            emg_offsets = np.append(
                emg_offsets, (1000 * (inhib[i, 1] - fac_i[i].max()) + t[np.argmax(fac_i[i])]))
        elif switches[i, -1] == 0:
            emg_offsets = np.append(
                emg_offsets, (1000 * (fac_i[i, -1] - inhib[i, -1]) + t[-1]))
...
def error_function_Go(params, data150, data125, data100, data_onsets, data_offsets):
    ...
    pred_onsets, pred_rates, pred_offsets = get_emg_onsets_offsets(
        t, fac_i, inhib_tonic)
...
def error_function_GS(params_GS, params_Go, data75, data50, data25):
    ...
    activation_thresholds = get_activation_thresholds(
        t, inhib_tonic, params_GS)
    pred75, pred50, pred25 = get_GS_tms_vals(
        t, fac_i, activation_thresholds, inhib_tonic)
...
def error_function_GS_facNew(params_facNew, activation_thresholds, components_Go, data_onsets, data_rates):
    ...
    pred_onsets, pred_rates = get_emg_onsets_facNew(
        t, fac_i_new, activation_thresholds)
    X2_onsets = get_chisquare(data_onsets, pred_onsets, nbins=2)[0]
    print "X2_onsets: ", X2_onsets
...
# /1000 to convert onset times into seconds, -0.8 to set relative to target line at 0ms
exp_EMG_onsets_three_stim = load_exp_data(fnameGoThreeStimOnly) / 1000 - .8
# sets EMG offset times based on empirical average burst duration of 107 ms
...
def build_Go_GS():
    ...
    params_Go = [a_facGo_mean, a_facGo_sd, b_facGo_mean,
                 b_facGo_sd, c_facGo_mean, c_facGo_sd, inhib_mean, inhib_sd]
    optGo = opt.minimize(error_function_Go, params_Go, args=(exp_MEPs_150, exp_MEPs_125, exp_MEPs_100,
                                                             exp_EMG_onsets_three_stim, sim_data_Go_EMG_offsets), method='Nelder-Mead', tol=0.01)
    print "ParamsOptimizedGo", optGo
    # optimized parameter output from Go optimization function
    params_Go = optGo.x
    ...
    optGS = opt.minimize(error_function_GS, params_GS, args=(
        components_Go, exp_GS_MEPs_75, exp_GS_MEPs_50, exp_GS_MEPs_25), method='Nelder-Mead', tol=0.01)
...
def build_GS_facNew():
    ...
    parameters (input manually). Sets EMG rate of onset for GS trials at 120% of Go trial values, 
    ...
    optFacNew = opt.minimize(error_function_GS_facNew, params_facNew, args=(
        activation_thresholds, components_Go, exp_GS_EMG_onsets_three_stim, sim_data_GS_rates), method='Nelder-Mead', tol=0.01)
